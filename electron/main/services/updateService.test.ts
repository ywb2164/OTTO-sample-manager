...
describe('UpdateService', () => {
  it('retries while latest.yml is still being uploaded', async () => {
    vi.useFakeTimers()
    try {
      ...
      expect(service.getState()).toMatchObject({ phase: 'up-to-date', message: null })
    } finally {
      vi.useRealTimers()
    }
  })
